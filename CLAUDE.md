...
- 原因：`Hybrid-组织知识` 节点按 `organization_id` 过滤，返回该组织下**所有项目**的文档
...
### 📚 智慧沉淀系统详解（2024-12-02 深度分析）

> 基于 `aip聊天生成历史2.0.json` 工作流和边缘函数代码的完整分析

#### 核心概念理解

**项目智慧库**：
- 基于用户在项目里聊天后产生的**集体智慧总结**
- 本质是"问过的问题不用重复问"
- 用于服务后面的用户，让 AI 回答更快更准
- 是一种**知识复用**机制

**组织智慧库**：
- 从多个项目智慧库中提取的**通用智慧**
- 不针对单独项目，而是跨项目的规律和洞察
- `project_id = NULL` 表示不属于任何单一项目
...
organization-wisdom-accumulation 边缘函数
    ↓ AI 归纳跨项目规律
组织智慧库 (documents 表, title='组织智慧库', project_id=NULL)
```
...
chat_history 表
    ↓
获取聊天记录（问题长度>10，答案长度>50）
    ↓
构建问答对 [{question, answer}, ...]
    ↓
发送给 GPT-4o-mini（带评估标准 Prompt）
    ↓
AI 返回 JSON：
[{
  "question": "优化后的问题",
...
生成向量嵌入（text-embedding-3-small）
    ↓
存入 documents 表（title='项目智慧库'）
```

**AI 提取标准**：
```
评估标准：
1. 问题应该是有价值的、可复用的（不是简单的闲聊）
2. 回答应该包含实质性的信息或见解
3. 问答对应该对项目其他成员有参考价值
```
...
| 对话类型 | 会被提取？ | 原因 |
|---------|-----------|------|
| "这个项目是做什么的？" → "这是一个关于..." | ✅ 是 | 有价值、可复用 |
| "今天天气怎么样" → "今天晴天" | ❌ 否 | 闲聊，无实质信息 |
| "帮我总结第三天的课程内容" → "第三天主要学习了..." | ✅ 是 | 包含具体知识 |
| "好的" → "收到" | ❌ 否 | 太短，无信息量 |
...
**详细流程**：
```
documents 表（title='项目智慧库'）
    ↓
获取该组织下所有项目的智慧（最近7天，最多50条）
    ↓
发送给 GPT-4o-mini（聚合 Prompt）
    ↓
AI 返回 JSON：
[{
  "topic": "项目管理最佳实践",
...
生成向量嵌入
    ↓
存入 documents 表（title='组织智慧库', project_id=NULL）
```

**什么内容才够"组织级智慧"？**

项目智慧（具体、针对单个项目）：
- 观音之道项目："第三天的课程重点是冥想技巧"
- 环保行动项目："垃圾分类的四个类别是..."
...
总结：通过分析多个项目的用户问答，发现以下规律...

关键洞察：
1. 用户在项目初期最常问"这个项目是做什么的"
2. 每日打卡能提升学习完成率
3. 小组讨论比独立学习效果更好
4. 视频资料比文字更受欢迎
```
...
| 知识类型 | 来源 | 存储位置 | 标识方式 | 当前状态 |
|---------|------|---------|---------|---------|
| **项目知识库** | 用户上传的文档 | `document_chunks` | `project_id=有值` | ✅ 已实现 |
| **组织知识库** | 管理员上传的通用文档 | `document_chunks` | `project_id=NULL` | ⏳ 架构已有，暂未使用 |
| **项目智慧库** | 聊天记录AI提取 | `documents` | `title='项目智慧库'` | ❌ 函数已有，未触发 |
| **组织智慧库** | 项目智慧AI聚合 | `documents` | `title='组织智慧库'` | ❌ 函数已有，未触发 |
...
│                    并行查询（4路）                        │
├─────────────┬─────────────┬─────────────┬───────────────┤
│ 项目知识库   │ 组织知识库   │ 项目智慧库   │ 组织智慧库     │
│ (上传文档)   │ (管理员上传) │ (聊天提取)   │ (智慧聚合)     │
└─────────────┴─────────────┴─────────────┴───────────────┘
                              ↓
                          合并结果
                              ↓
...
**不会变慢！** 因为是并行执行：
```
串行执行（慢）：查询1→查询2→查询3→查询4 = 4秒
并行执行（快）：查询1/查询2/查询3/查询4 同时进行 = 约1秒
```

**预估耗时**：
- 当前（2路知识库）：~2-3秒
- 加入智慧库（4路并行）：~2-3秒（不增加时间）
- 主要耗时还是 AI 生成回答
