...
│                   并行执行 (4路)                      │
├────────────┬───────────┬───────────┬─────────────────┤
│ Embeddings │ 用户名    │ 用户画像  │ 最近5条聊天记录  │
│ (HTTP Req) │ (Postgres)│ (Postgres)│ (Postgres)      │
└────────────┴───────────┴───────────┴─────────────────┘
    ↓
HTTP Request → hybrid_search_gaia (返回子块 + 父文档内容)
    ↓
Cohere Rerank (可选，重排序提升相关性)
    ↓
Code (整合上下文：知识库 + 用户画像 + 聊天记录)
    ↓
Basic LLM Chain (Gemini，快速响应)
    ↓
Respond to Webhook
```
...
## 关键 RPC 函数
- `hybrid_search` - AIP 混合搜索（查询 `documents` 表，支持多项目 ID、区分项目/组织文档）
