...
export default function ListeningCoursePage() {
  ...
  useEffect(() => {
    if (selectedLesson) {
      setFormData({
        ...
        original_text: selectedLesson.original_text || '',
        deep_interpretation: selectedLesson.deep_interpretation || '',
        meditation_guide: selectedLesson.meditation_guide || '',
        life_practice: selectedLesson.life_practice || ''
      })
      loadMediaResources(selectedLesson.id)
    }
  }, [selectedLesson])
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    if (!selectedLesson || !event.target.files || event.target.files.length === 0) return
    ...
    try {
      ...
      const fileName = `${selectedLesson.id}_${Date.now()}.${fileExt}`
      ...
      alert('文件上传成功！')
      await loadMediaResources(selectedLesson.id)
    } catch (error) {
  ...
  const handleAddNewDay = async () => {
    const newDayNumber = lessons.length > 0
      ? Math.max(...lessons.map(l => l.day_number)) + 1
      : 1
              ...
              {lessons.map((lesson) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-white font-medium">第 {lesson.day_number} 天</p>
                      <p className="text-gray-400 text-sm mt-1 line-clamp-1">{lesson.title}</p>
                    </div>
          ...
          {selectedLesson ? (
            <div className="max-w-4xl mx-auto p-8">
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-bold text-white">第 {selectedLesson.day_number} 天</h2>
                <button
                  onClick={handleSave}
                  disabled={saving}
