...
const COURSE_SUBTITLE = '克里希那穆提《生命之书》六月主题 · 30天能量炼金之旅'
const THEME_COLOR = 'teal'
const STORAGE_PATH = 'energy-alchemy'

export default function EnergyAlchemyCoursePage() {
    ...
    return (
      <div className="min-h-screen bg-cosmic-void flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-teal-300 mt-4">加载中...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-cosmic-void relative overflow-hidden">
      <div className="absolute inset-0 overflow-hidden">
        {isMounted && particles.map((particle) => (
          <motion.div key={particle.id} className="absolute w-1 h-1 bg-teal-400 rounded-full opacity-30"
        ...
        <div className="w-80 bg-white/5 backdrop-blur-md border-r border-white/10 overflow-y-auto">
          <div className="p-4">
            <div className="space-y-2 mb-4">
              <button onClick={() => router.push('/admin/courses/energy-alchemy/students')}
                className="w-full px-4 py-3 bg-white/5 hover:bg-white/10 border border-white/10 hover:border-teal-500/50 rounded-lg transition-all flex items-center gap-3 text-left">
            ...
            <button onClick={handleAddNewDay}
              className="w-full mb-4 px-4 py-3 bg-gradient-to-r from-teal-600 to-cyan-600 hover:from-teal-700 hover:to-cyan-700 text-starlight rounded-lg font-medium transition-all flex items-center justify-center gap-2">
              <Plus className="w-5 h-5" /> 新增一天
            </button>
            <div className="space-y-2">
              {courseContents.map((content) => (
                <div key={content.id}
                  className={`relative rounded-lg transition-all ${selectedContent?.id === content.id ? 'bg-teal-600/30 border border-teal-500/50' : 'bg-white/5 border border-white/10 hover:bg-white/10'}`}>
            ...
            <div className="w-full max-w-none p-8 pb-32">
              ...
              <div className="mt-12 pt-8 border-t border-white/10">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <label className="px-6 py-2 bg-teal-600 hover:bg-teal-700 text-starlight rounded-lg font-medium transition-all flex items-center gap-2 cursor-pointer">
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          {editingMediaId !== media.id && (
                            <div className="flex items-center gap-2 ml-4">
                              ...
                              <label className="p-2 bg-teal-600/20 hover:bg-teal-600/40 text-teal-400 rounded transition-all cursor-pointer" title="替换文件">
