...
**规则**：
1. COURSE_TYPE必须是：listening、earth、pbl 之一（小写）
2. TITLE是课程标题（一行）
  ...
  try {
    ...
    if (documentContent.length > MAX_CHUNK_SIZE && courseType === 'listening') {
      ...
      if (matches.length >= 2) {
        ...
        if (result1) {
          const parsed1 = parseDelimitedText(result1)
        ...
        if (result2) {
          const parsed2 = parseDelimitedText(result2)
        ...
        if (response1.text) {
          const parsed1 = parseDelimitedText(response1.text)
        ...
        if (response2.text) {
          const parsed2 = parseDelimitedText(response2.text)
          if (parsed2 && parsed2.length > 0) {
            const offset = allContents.length
            parsed2.forEach((content: any) => {
              content.sequence_number = content.sequence_number + offset
      ...
      // Listening课程使用分隔符格式，其他课程使用JSON
      const isListening = courseType === 'listening'
      const prompt = isListening ? LISTENING_PROMPT + documentContent : systemPrompt
