...
import { updateOrganization } from '@/lib/aip/api'
...
export function EditOrganizationModal({ organization, onClose, onSuccess }: EditOrganizationModalProps) {
  ...
  const [isPublic, setIsPublic] = useState(organization.is_public || false)
      ...
      <div className="bg-white/5 backdrop-blur-xl border border-white/20 rounded-2xl max-w-md w-full p-6 shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex items-center justify-between p-4 bg-white/5 border border-white/10 rounded-lg">
            ...
            >
              <span
                className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform left-1 ${
