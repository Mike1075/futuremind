    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        <div className="flex items-center justify-center w-10 h-10 bg-gradient-to-br from-orange-500 to-amber-500 rounded-lg">
          <svg className="w-6 h-6 text-starlight" fill="currentColor" viewBox="0 0 20 20">
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
          </svg>
        </div>
        <div>
          <h3 className="text-h2 font-bold text-starlight">优秀作业展示</h3>
          <p className="text-small text-starlight-muted">来自同学们的优秀作品（评分≥90分）</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {submissions.map((submission, index) => (
          <motion.div
            ...
            className="card-glass p-6 hover:border-orange-500/50 transition-all"
          >
            {/* 头部：学生信息和评分 */}
            <div className="flex items-center justify-between mb-4 pb-4 border-b border-white/10">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-cyan-500 rounded-full flex items-center justify-center">
                  <span className="text-starlight font-semibold text-small">
                    {submission.studentName.charAt(0)}
                  </span>
                </div>
                <div>
                  <p className="font-semibold text-starlight">{submission.studentName}</p>
                  <p className="text-xs text-starlight-dim">{formatDate(submission.submittedAt)}</p>
                </div>
              </div>
            ...
            {/* 作业内容 */}
            <div className="mb-4">
              <p className="text-starlight text-small leading-relaxed whitespace-pre-wrap">
                {submission.content}
              </p>
            </div>
            ...
            {submission.attachments && submission.attachments.length > 0 && (
              <div className="mt-4 grid grid-cols-2 gap-2">
                {submission.attachments.filter(att => att.type === 'image').map((attachment, idx) => (
                  <div
                    key={idx}
                    className="relative aspect-video rounded-lg overflow-hidden bg-cosmic-void border border-white/10"
                  >
                    <Image
                      src={attachment.url}
                      alt={attachment.name}
