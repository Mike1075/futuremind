-- 修复意识树触发器的HTTP调用
-- 创建时间: 2024-12-27
-- 描述: 修复触发器中的net.http_post函数错误，使用正确的extensions.http_post
...
BEGIN
    ...
    IF should_evaluate THEN
        BEGIN
            ...
            INSERT INTO trigger_log (message, conversation_id)
            VALUES (
                FORMAT('Consciousness tree evaluation triggered for user: %s (message count: %s) - HTTP Status: %s', NEW.user_id, user_messages, http_result.status),
                NEW.id
            );
...
DO $$
BEGIN
    RAISE NOTICE '意识树触发器HTTP调用修复完成！';
    RAISE NOTICE '- 使用正确的extensions.http_post函数';
    RAISE NOTICE '- 添加了HTTP错误处理';
    RAISE NOTICE '- 触发器现在应该能正常调用边缘函数了';
END $$;